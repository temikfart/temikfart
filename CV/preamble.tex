% Change the page layout if you need to
\geometry{left=1cm,right=1cm,top=1.5cm,bottom=1cm,columnsep=1.2cm}

% The paracol package lets you typeset columns of text in parallel
\usepackage{paracol}

% Change the font if you want to, depending on whether
% you're using pdflatex or xelatex/lualatex
\ifxetexorluatex
% If using xelatex or lualatex:
\setmainfont{Roboto Slab}
\setsansfont{Lato}
\renewcommand{\familydefault}{\sfdefault}
\else
% If using pdflatex:
\usepackage[rm]{roboto}
\usepackage[defaultsans]{lato}
% \usepackage{sourcesanspro}
\renewcommand{\familydefault}{\sfdefault}
\fi

% Change the colours if you want to
\definecolor{SlateGrey}{HTML}{2E2E2E}
\definecolor{LightGrey}{HTML}{666666}
\definecolor{DarkBlue}{HTML}{2F69A2}
\definecolor{Blue}{HTML}{4785C3}
\definecolor{LightBlue}{HTML}{0082EE}
\definecolor{GoldenEarth}{HTML}{E7D192}
\colorlet{name}{black}
\colorlet{tagline}{Blue}
\colorlet{heading}{DarkBlue}
\colorlet{headingrule}{GoldenEarth}
\colorlet{subheading}{Blue}
\colorlet{accent}{Blue}
\colorlet{emphasis}{SlateGrey}
\colorlet{body}{LightGrey}
\colorlet{url}{LightBlue}

% Change some fonts, if necessary
\renewcommand{\namefont}{\Huge\rmfamily\bfseries}
\renewcommand{\personalinfofont}{\footnotesize}
\renewcommand{\cvsectionfont}{\LARGE\rmfamily\bfseries}
\renewcommand{\cvsubsectionfont}{\large\bfseries}


% Change the bullets for itemize and rating marker
% for \cvskill if you want to
\renewcommand{\itemmarker}{{\small\textbullet}}
\renewcommand{\ratingmarker}{\faCircle}

% Version watermark in the corner
\usepackage[angle=0, fontsize=0.015\paperwidth, text=\cvversion, hpos=0.99\paperwidth, vpos=0.01\paperwidth, anchor=rt]{draftwatermark}

% Text inside mathmode
\usepackage{amsmath}

% Huawei logo
\newcommand*\huawei{\raisebox{-0.15\totalheight}{\includegraphics[width=15pt]{images/huawei}}\hspace{.5ex}}

% ISP RAS logos
\newcommand*\isprasru{\raisebox{-0.07\totalheight}{\includegraphics[width=30pt]{images/ispras_ru}}\hspace{.5ex}}
\newcommand*\isprasen{\raisebox{-0.07\totalheight}{\includegraphics[width=30pt]{images/ispras_en}}\hspace{.5ex}}


% MIPT logo
\newcommand*\mipt{\raisebox{-0.15\totalheight}{\includegraphics[width=10pt]{images/mipt}}\hspace{.5ex}}